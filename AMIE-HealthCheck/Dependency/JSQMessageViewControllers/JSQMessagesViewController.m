...
- (void)jsq_configureMessagesViewController
{
    ...
    [self.inputToolbar.contentView.micButton setImage:[UIImage imageNamed:@"ic_mic"] forState:UIControlStateNormal];

    [self.inputToolbar.contentView.micButton addTarget:self action:@selector(didClickOnMic:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if (self.inputToolbar.contentView.textView != nil) {
        self.keyboardController = [[JSQMessagesKeyboardController alloc] initWithTextView:self.inputToolbar.contentView.textView
                                                                     ...
                                                                     panGestureRecognizer:self.collectionView.panGestureRecognizer
                                                                                 delegate:self];
        [self makeTextViewTextCenter];
    }
    
    
}

-(void)makeTextViewTextCenter {
    UITextView *tv = self.inputToolbar.contentView.textView;
//    CGFloat topCorrect = ([tv bounds].size.height - [tv contentSize].height * [tv zoomScale])/2.0;
//    topCorrect = ( topCorrect < 0.0 ? 0.0 : topCorrect );
//    tv.contentOffset = (CGPoint){.x = 0, .y = -topCorrect};
    CGSize contentSize = [tv sizeThatFits:CGSizeMake(tv.bounds.size.width, CGFLOAT_MAX)];
    CGFloat topCorrection = (tv.bounds.size.height - contentSize.height * tv.zoomScale) / 2.0;
    tv.contentOffset = CGPointMake(0, -topCorrection);
}
- (void)dealloc
{
...
- (UICollectionViewCell *)collectionView:(JSQMessagesCollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BOOL isMediaMessage = [messageItem isMediaMessage];
     ...
     BOOL isHasLink = [messageItem hasBelowView];
    ...
    if (isHasLink) {
        cell.amieTopBorderView.hidden = false;
        cell.amieBottomView.hidden = false;
        cell.amieHeightConstrain.constant = 60.0f;
       [cell.amieBottomContentLbl addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleLinkDetction:)]];
        cell.amieBottomContentLbl.tag = indexPath.item;
        
        if ([[messageItem viewTypeName] isEqualToString:@"Link"]) {
            cell.amieBottomIcon.image = [UIImage imageNamed:@"healthLinkIcon"];
             cell.amieBottomContentLbl.attributedText = [messageItem content];
          
            
        }else
        {
            cell.amieBottomIcon.image = [UIImage imageNamed:@"callIcon"];
             cell.amieBottomContentLbl.attributedText = [messageItem content];
        }
    }else
    {
        cell.amieTopBorderView.hidden = true;
        cell.amieBottomView.hidden = true;
        cell.amieHeightConstrain.constant = 0;
    }
    ...
    if (isOutgoingMessage) {
        ...
        cell.cellTopLabel.textAlignment = NSTextAlignmentRight;
    }
    else {
...
- (void)textViewDidBeginEditing:(UITextView *)textView
{
    ...
    [self.inputToolbar.contentView.leftBarButtonItem setImage:[UIImage imageNamed:@"selectedKeyboard"] forState:UIControlStateNormal];
    if (self.automaticallyScrollsToMostRecentMessage) {
...
- (void)handleLinkDetction:(UITapGestureRecognizer *)gesture {
    ...
    __block NSURL *URL;
    [messageItem.content enumerateAttribute:NSLinkAttributeName inRange:NSMakeRange(0, Label.attributedText.length) options:0 usingBlock:^(NSURL *url, NSRange range, BOOL *stop) {
        if (url) {
            URL = url;
        }
    }];
    ...
    NSLog(@"%@", URL);
    if ([URL.absoluteString containsString:@"www.heart.org"]) {
        [Utilities openApplicationWithURL:URL.absoluteString];
    }else {
